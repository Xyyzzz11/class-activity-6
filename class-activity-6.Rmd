...
DF1<-unite(DF1,Student_Name, First.Name, Last.Name, sep = " ")
rownames(DF1)<-DF1$Student_Name
DF1<- DF1[-1]
#Wrangle data using dplyr to include only the numerical values.
DF2<- select(DF1,1:11)
DF2 <- DF2 %>% mutate_all(funs(gsub("[a-zA-Z]", "", .)))%>% mutate_all(funs(as.numeric(.)))
